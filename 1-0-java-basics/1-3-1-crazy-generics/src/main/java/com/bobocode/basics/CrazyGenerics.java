...
public class CrazyGenerics {
    ...
    public interface Converter<T, R> { // todo: introduce type parameters
        public R convert(T t); // todo: add convert method
    }
    ...
    static class CollectionUtil {
        ...
        public static <T extends BaseEntity> boolean hasNewEntities(Collection<? extends BaseEntity> entities) {
            for(BaseEntity entity : entities) {
                if(entity.getUuid() == null) return true;// todo: refactor parameter and implement method
            }
            return false;
        }
        ...
        public static boolean isValidCollection(Collection<? extends BaseEntity> entities, Predicate<? super BaseEntity> validator) {
            return entities.stream()
                    .filter(validator)
                    .count() == entities.size();// todo: add method parameters and implement the logic
        }
        ...
        public static <T extends BaseEntity> boolean hasDuplicates(Collection<T> entities, T targetEntity) {
            return entities.stream()
                    .filter(element -> element.getUuid() == targetEntity.getUuid())
        ...
        public static <T extends BaseEntity> Optional<T> findMax(Iterable<T> iterable, Comparator<T> comparator) {
            Collection<T> iterableCollection = (Collection<T>) iterable;
            return iterableCollection.stream().max(comparator);
        }
        ...
        public static <T extends BaseEntity> T findMostRecentlyCreatedEntity(Collection<T> entities) {
            Optional<BaseEntity> optEntity = findMax((Iterable<BaseEntity>) entities, CREATED_ON_COMPARATOR);
            return (T) optEntity.get();
        }
        ...
        public static void swap(List<?> elements, int i, int j) {
            ...
            Object tempElement = elements.get(i);

            try {
                java.lang.reflect.Method setMethod = List.class.getMethod("set", int.class, Object.class);

                setMethod.invoke(elements, i, elements.get(j));
                setMethod.invoke(elements, j, tempElement);
            } catch (Exception e) {
                throw new RuntimeException("Swap operation failed", e);
