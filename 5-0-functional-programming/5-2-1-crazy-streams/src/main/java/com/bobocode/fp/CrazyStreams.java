...
public class CrazyStreams {
    ...
    private static final String BALANCE_BY_EMAIL_NOT_FOUND = "Cannot find Account by email=";
    ...
    public Optional<Account> findRichestPerson() {
        return accounts.stream()
                .max((account1, account2) -> {
                    ...
                    return comparator.compare((Account) account1, (Account) account2);
                });
    }
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream()
                // partitioningBy is used when we want to divide the stream into to groups and the collect them into a hashMap
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream()
                // partitioningBy is used when we want to divide the stream into several groups and the collect them into a hashMap
    ...
    public int getNumOfLettersInFirstAndLastNames() {
        return accounts.stream()
                .map(account -> lettersCount(account.getFirstName()) + lettersCount(account.getLastName()))
                .reduce(0, Integer::sum);
    }

    private int lettersCount(String input) {
    ...
    public List<Account> sortByFirstAndLastNames() {
        return accounts.stream()
                .sorted((account1, account2) -> {
                    int result = account1.getFirstName().compareTo(account2.getFirstName());
                    if(result == 0) return account1.getLastName().compareTo(account2.getLastName());
                    return result;
                })
                .toList();
    }
    ...
    public boolean containsAccountWithEmailDomain(String emailDomain) {
        return accounts.stream()
                .anyMatch(account -> account.getEmail().substring(account.getEmail().indexOf('@') + 1).equals(emailDomain));
    }
    ...
    public BigDecimal getBalanceByEmail(String email) {
        return accounts.stream()
                ...
                .map(Account::getBalance)
                .findFirst()
                .orElseThrow(() -> new EntityNotFoundException(BALANCE_BY_EMAIL_NOT_FOUND + "fake@mail.com"));
    }
    ...
    public Map<String, Set<String>> groupFirstNamesByLastNames() {
        return accounts.stream()
                .collect(Collectors.groupingBy(Account::getLastName, TreeMap::new, mapping(Account::getFirstName, toSet())));
    }
    ...
    public Map<Month, BigDecimal> groupTotalBalanceByCreationMonth() {
        return accounts.stream()
                .collect(Collectors.groupingBy(
                        account -> account.getCreationDate().getMonth(), // Clave: mes de creación
                        Collectors.reducing(
                                BigDecimal.ZERO, // Valor inicial
                                Account::getBalance, // Función de mapeo
                                BigDecimal::add // Función de reducción: suma los balances
                        )
                ));
    }
    ...
    public Map<Character, Long> getCharacterFrequencyInFirstNames() {
        return accounts.stream()
                .flatMap(account -> account.getFirstName()
                        .chars()
                        .mapToObj(c -> (char) c))
                .collect(Collectors.groupingBy(
                        (c) -> c,
    ...
    public Map<Character, Long> getCharacterFrequencyIgnoreCaseInFirstAndLastNames(int nameLengthBound) {
        return accounts.stream()
                .flatMap(account -> ((account.getFirstName().length() >= nameLengthBound ? account.getFirstName() : "") + (account.getLastName().length() >= nameLengthBound ? account.getLastName() : ""))
                        .chars()
                        .mapToObj(c -> (char) c))
                .collect(Collectors.groupingBy(
