...
public class CrazyOptionals {
    ...
    public static void deposit(AccountProvider accountProvider, BigDecimal amount) {
        accountProvider.getAccount().ifPresent(account -> account.setBalance(BigDecimal.valueOf(account.getBalance().intValue() + amount.intValue())));
    }
    ...
    public static Optional<BigDecimal> retrieveBalance(AccountProvider accountProvider) {
        return Optional.ofNullable(accountProvider.getAccount())
                .flatMap(optionalAccount -> optionalAccount.map(Account::getBalance));
    }
    ...
    public static Optional<BigDecimal> retrieveCreditBalance(CreditAccountProvider accountProvider) {
            return Optional.ofNullable(accountProvider.getAccount())
                    .flatMap(optionalAccount -> optionalAccount.flatMap(CreditAccount::getCreditBalance));
    }
    ...
    public static Optional<Account> retrieveAccountGmail(AccountProvider accountProvider) {
        AtomicReference<Optional<Account>> atomicAccount = new AtomicReference<>();
        accountProvider.getAccount().filter(account -> account.getEmail().endsWith("gmail.com"))
                .ifPresentOrElse(
                        (account -> atomicAccount.set(Optional.of(account))),
                        () -> atomicAccount.set(Optional.empty())
                );
        return atomicAccount.get();
    }
    ...
    public static Account getAccountWithMaxBalance(List<Account> accounts) {
        return accounts.stream()
                .max(Comparator.comparingDouble(account -> account.getBalance().doubleValue()))
                .orElseThrow(NoSuchElementException::new);
    }
